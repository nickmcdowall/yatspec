...
public class TestParser {
    ...
    private static Option<URL> getJavaSourceFromClassPath(Class aClass) {
        return isObject(aClass) ? NO_URL : option(aClass.getClassLoader().getResource(Files.toJavaResourcePath(aClass)));
    }

    private static Option<URL> getJavaSourceFromFileSystem(Class aClass) {
        return isObject(aClass) ? NO_URL : recursiveFiles(workingDirectory()).find(where(path(), Strings.endsWith(Files.toJavaPath(aClass)))).map(toURL());
    }
