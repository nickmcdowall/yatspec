...
public class LinkingNoteRendererTest {
    ...
    private String theRenderedValueOfTheLinkingNoteOfThisMethod() throws Exception {
        ...
        LinkingNote linkingNote = (
                methods.get(0).getAnnotations().stream()
                        .filter(annotation -> annotation instanceof LinkingNote)
                        .map(annotation -> ((LinkingNote) annotation))
                        .findFirst()
                        .get()
        );
        return new LinkingNoteRenderer(LinkingNoteRendererTest.class).render(linkingNote);
    }
}
